...
class Space {
    ...
    fun addValue(i: Int, j: Int, planetImage: ConvertPlanetImage.CommandImage, planetInfect: Int, satiety: Int, age: Int) {
    ...
    fun addFoodUser(i: Int, j: Int, foodCommand: ConvertFoodImage.CommandFood, satiety: Int) {
    ...
    fun foodChange(i: Int, j: Int, foodImage: Drawable, satiety: Int) {
        ...
        if (food == null) {
            myFoodList.add(Food(x = i, y = j, foodImage = convertFoodImage.food_M , satiety = satiety))
            spaceListener?.changeSpace(space = this)
        }
